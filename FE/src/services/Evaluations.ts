...
export const evaluationService = {
  getEvaluationById: async (id: string): Promise<Evaluation> => {
    const response: AxiosResponse<Evaluation> = await api.get(`/api/Evaluations/${id}`);
    return response.data;
  },

  // PUT /api/Evaluations/{id}
  updateEvaluation: async (id: string, data: UpdateEvaluationRequest): Promise<Evaluation> => {
    const response: AxiosResponse<Evaluation> = await api.put(`/api/Evaluations/${id}`, data);
    return response.data;
  },
  ...
  getAllEvaluations: async (): Promise<Evaluation[]> => {
    const response: AxiosResponse<Evaluation[]> = await api.get('/api/Evaluations');
    return response.data;
  },

  // POST /api/Evaluations
  createEvaluation: async (data: CreateEvaluationRequest): Promise<Evaluation> => {
    const response: AxiosResponse<Evaluation> = await api.post('/api/Evaluations', data);
    return response.data;
  },

  // GET /api/Evaluations/project/{projectId}
  getEvaluationsByProject: async (projectId: number): Promise<Evaluation[]> => {
    const response: AxiosResponse<Evaluation[]> = await api.get(`/api/Evaluations/project/${projectId}`);
    return response.data;
  },

  // GET /api/Evaluations/task/{taskId}
  getEvaluationsByTask: async (taskId: number): Promise<Evaluation[]> => {
    const response: AxiosResponse<Evaluation[]> = await api.get(`/api/Evaluations/task/${taskId}`);
    return response.data;
  },

  // GET /api/Evaluations/evaluated-by/{userId}
  getEvaluationsByUser: async (userId: string): Promise<Evaluation[]> => {
    const response: AxiosResponse<Evaluation[]> = await api.get(`/api/Evaluations/evaluated-by/${userId}`);
    return response.data;
  },

  // GET /api/Evaluations/type/{type}
  getEvaluationsByType: async (type: string): Promise<Evaluation[]> => {
    const response: AxiosResponse<Evaluation[]> = await api.get(`/api/Evaluations/type/${type}`);
    return response.data;
  },
};
