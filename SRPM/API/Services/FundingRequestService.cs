        ...
        public async Task<FundingRequestDto?> CreateAsync(CreateFundingRequestRequest request, int userId)
        {
            ...
            if (project.OwnerId != userId)
            {
                var notification = new Notification
                {
                    ...
                    Message = $"Một yêu cầu tài trợ mới đã được gửi cho đề tài  '{project.Title}': {fundingRequest.Title}",
                    Type = "Info",
        ...
        public async Task<bool> ApproveAsync(int id, ApproveFundingRequestRequest request, int userId)
        {
            ...
            if (success)
            {
                ...
                var notification = new Notification
                {
                    ...
                    Message = $"Chúc mừng, yêu cầu tài trợ của bạn '{fundingRequest.Title}' đã được duyệt !.",
                    Type = "Success",
                    RelatedEntityType = "FundingRequest",
                    RelatedEntityId = fundingRequest.Id,
                ...
                if (project != null && project.OwnerId != fundingRequest.RequestedById)
                {
                    ...
                    {
                        Title = "Funding Request Approved",
                        Message = $"Chúc mừng, Yêu cầu tài trợ của bạn cho dự án '{project.Title}' đã được duyệt bởi: {fundingRequest.Title}",
                        Type = "Success",
                        RelatedEntityType = "FundingRequest",
                        RelatedEntityId = fundingRequest.Id,
                        UserId = project.OwnerId
        ...
        public async Task<bool> RejectAsync(int id, RejectFundingRequestRequest request, int userId)
        {
            ...
            if (success)
            {
                ...
                var notification = new Notification
                {
                    ...
                    Message = $"Yêu cầu tài trợ của bạn cho dự án '{fundingRequest.Title}' không được duyệt. Lý do: {request.Reason}, Chúc bạn may mắn lần sau",
                    Type = "Error",
                    RelatedEntityType = "FundingRequest",
                    RelatedEntityId = fundingRequest.Id,
                    UserId = fundingRequest.RequestedById
                ...
                if (project != null && project.OwnerId != fundingRequest.RequestedById)
                {
                    ...
                    {
                        Title = "Funding Request Rejected",
                        Message = $"Yếu cầu tài trợ của bạn cho dự án'{project.Title}' không được duyệt bởi: {fundingRequest.Title}",
                        Type = "Error",
                        RelatedEntityType = "FundingRequest",
                        RelatedEntityId = fundingRequest.Id,
                        UserId = project.OwnerId
