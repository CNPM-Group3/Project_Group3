...
namespace SRPM.API.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        [Authorize(Roles = "Admin")]
        [SwaggerOperation(Summary = "Sửa vai trò của người dùng")]
        public async Task<ActionResult> UpdateUserRoles(int userId, [FromBody] UpdateUserRolesRequest request)
        {
            var success = await _userService.UpdateUserRolesAsync(userId, request.RoleNames);
            if (!success)
                return BadRequest("Không thể cập nhật vai trò người dùng hoặc vai trò không hợp lệ");
