...
interface NavigationItem {
  ...
  label: string;
  path: string;
  mainPath?: string; // Thêm mainPath để xác định trang chính của mục
}
...
const ThanhVienNghienCuuSidebar: React.FC = () => {
  ...
  const handleNavigation = (item: NavigationItem) => {
    ...
    // Luôn điều hướng về trang chính của mục
    if (item.path === "/duan" || item.path === "/taitro") {
      navigate(item.path);
    } else {
      navigate(item.path);
    }
...
export default ThanhVienNghienCuuSidebar; 